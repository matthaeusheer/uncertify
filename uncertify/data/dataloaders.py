...
def dataloader_factory(dataset_type: DatasetType, batch_size: int, transform: Any = None, num_workers: int = 0,
    ...
    if dataset_type == DatasetType.MNIST:
        ...
        return None, brats17_val_dataloader(batch_size, False, num_workers, transform)
    elif dataset_type == DatasetType.CAMCAN:
        return (camcan_train_data_loader(batch_size, shuffle_train, num_workers, transform),
                camcan_val_data_loader(batch_size, num_workers, transform))
    else:
...
def camcan_train_data_loader(batch_size: int, shuffle: bool, num_workers: int = 0, transform: Any = None) -> DataLoader:
    ...
    camcan_train_dataloader = DataLoader(camcan_train_dataset, batch_size=batch_size,
                                         shuffle=shuffle, num_workers=num_workers)
    return camcan_train_dataloader


def camcan_val_data_loader(batch_size: int, num_workers: int = 0, transform: Any = None) -> DataLoader:
